...
Instead of choosing heuristics identically for all nodes in the search tree, we thought about choosing heuristics adaptively based on current situation. For instance, if lengths of clauses do not vary much, it would be better to just apply DLCS rather than strict Jerowslow-Wang heuristics. We may also try ensemble of heuristics; if two literals have similar Jerowslow-Wang scores, then we may better consult other heuristics instead of strictly following the one with similar scores. 
